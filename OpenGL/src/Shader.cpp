...
{
    ShaderProgramSource source = ParseShader(filepath); // Gets vertex and fragment shader source code
    m_RendererID = CreateShader(source.VertexSource, source.FragmentSource); // Create shader
}
...
{
    std::ifstream stream(filepath); // Opens up Basic.shader file
...
unsigned int Shader::CreateShader(const std::string& vertexShader, const std::string& fragmentShader)
{
    ...
    unsigned int vs = CompileShader(GL_VERTEX_SHADER, vertexShader); // Compiled vertex shader
    unsigned int fs = CompileShader(GL_FRAGMENT_SHADER, fragmentShader); // Compiled fragment shader

    glAttachShader(program, vs); // Attach vertex shader into program
    glAttachShader(program, fs); // Attach fragment shader into program
    glLinkProgram(program); // link program
    glValidateProgram(program);

    glDeleteShader(vs); // Delete vertex shader obj
    glDeleteShader(fs); // Delete fragment shader obj

    return program; // Return Shader
}
